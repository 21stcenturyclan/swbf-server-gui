...
class SWBFWindow(QMainWindow):
    def __init__(self, w, h):
        ...
        self.tabs.addTab(ServerTab('Server 1', self.tabs, 0), 'Server 1')
        ...
        self._main_grid.addWidget(self.tabs)
        ...
        file.addAction(QAction('&Exit', self))

        tool = self.addToolBar('Run')
        ...
        tool.addWidget(self.run)
        tool.addWidget(self.stop)
    ...
    def _start_server(self):
        self.runner.add_process(self._file_select_value.text(), self.tabs.currentWidget().options())
        self.run.setDisabled(True)
    ...
    def _tab_changed(self, index):
        if index == self.tabs.count() - 1:
            ...
            new_tab = ServerTab(tab_name, self.tabs, new_index + 1)
            self.tabs.addTab(new_tab, tab_name)
            self.tabs.setTabOrder(new_tab, self.add_tab)
